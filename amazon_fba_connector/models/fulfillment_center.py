...
class amazon_fulfillment_center(models.Model):
    ...
    seller_id = fields.Many2one('amazon.seller.ept',string='Amazon Seller', compute=_get_seller_id, store=True, readonly=True)
    center_code = fields.Char(size=50, string='Fulfillment Center Code', required=True)
    warehouse_id = fields.Many2one('stock.warehouse', string='Warehouse')
    
    _sql_constraints=[('fulfillment_center_unique_constraint','unique(seller_id,center_code)',"Fulfillment center must be unique by seller.")]