...
class amazon_prepare_product_wizard(models.TransientModel):
    ...
    def export_products_to_csv(self,product_ids=[]):
        ...
        return {
                ...
                'url':   '/web/binary/download_document?model=amazon.product.wizard&field=datas&id=%s&filename=%s'%(rec.id,filename),
                'target': 'self',
                }
...
#         search_keywords=",".join(product.search_term_ids.mapped('name'))
        ...
        if product.product_id.ept_image_ids.ids:
            main_images=self.env['product.image.ept'].search([('image_type','=','Main'),('id','in',product.product_id.ept_image_ids.ids)])
        ...
        if product.bullet_point_ids.ids:
            ...
            for x in range(0,len(product.bullet_point_ids.ids)):                
                bullent_point_dict.update({'bullet_point'+str(count):list(reversed(product.bullet_point_ids.mapped('name')))[x]})
                count+=1
                if count>5:
                    break
        return {'SKU':product.seller_sku,
                ...
                'Search Keywords':product.search_term_ids and product.search_term_ids.mapped('name')[0] or '',    
