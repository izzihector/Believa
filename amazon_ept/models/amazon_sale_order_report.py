...
from odoo.addons.amazon_ept.amazon_emipro_api.mws import Reports,DictWrapper,Orders
import base64
...
class sale_order_report_ept(models.Model):
    _name="amazon.sale.order.report.ept"
    ...
    amazon_fbm_sale_order_ids=fields.One2many('sale.order','sale_order_report_id',string="Fbm orders")
...
#     instance_id=fields.Many2one('amazon.instance.ept',string="Instance")
    ...
    def create(self,vals):    
        try:
            sequence=self.env.ref('amazon_ept.seq_mfn_sales_report')
            if sequence:
    ...
    def on_change_seller_id(self):
        value = {}
        if self.seller_id:    
            value.update({'start_date':self.seller_id.order_last_sync_on,'end_date':datetime.now()})
        return {'value': value}
    ...
    def get_report(self):
        ...
        if hasattr(mws_obj,'response') and hasattr(mws_obj.response,'status_code') and mws_obj.response.status_code!=400:
            ...
            attachment = self.env['ir.attachment'].create({
                                               ...
                                               'res_model': 'amazon.sale.order.report.ept',
                                               'res_id': self.id,
                                               'type': 'binary'
                                             })
...
#         seller_sku and domain.append(('seller_sku','=',seller_sku))
    ...
    @api.multi
    def reprocess_sale_order_report_file(self):
        amazon_transaction_obj=self.env['amazon.transaction.log']
        model_id=amazon_transaction_obj.get_model_id('amazon.sale.order.report.ept')
        records=amazon_transaction_obj.search([('model_id','=',model_id),('action_type','!=','create'),('res_id','=',self.id)])
        records.unlink()
        self.button_process_sale_order_report()
    ...
    def button_process_sale_order_by_flat_report(self):
        ...
        for line in reader:
            ...
            already_exist_amazon_order=sale_order_obj.search([('amz_instance_id','in',instance_ids),('amazon_reference','=',order_id)])
            if already_exist_amazon_order:
                continue
    ...
    def process_sale_order_by_flat_report_file(self,order_details={},amazon_orders_reference=[]):
        ...
        seller=self.seller_id
        for x in range(0, len(amazon_orders_reference),50):
            odoo_order_ids = []
            auto_process_orders=[]
            cancel_orders_list=[]
            shipped_orders_dict={}
            ...
            for amazon_order_ref in orders:
                ...
                if not order_detail:
                    continue
                unshipped_orders_list=list(unshipped_orders_dict.keys())
                shipped_orders_list=list(shipped_orders_dict.keys())
    ...
    def create_sale_order_by_flat_report(self,amazon_order_ref,order_data,instance):
        ...
        amazon_order = sale_order_obj.create(order_vals)
        for order_line in order_lines:
            sale_line_obj.create_sale_order_line_by_flat_report(order_line,instance,amazon_order)
    ...
    def create_sales_order_vals_by_flat_report(self,partner_dict,order,instance,amazon_order_ref):
        ...
        ordervals={
                   ...
                   'global_channel_id':instance.seller_id.global_channel_id and instance.seller_id.global_channel_id.id or False
                   }
        ...
        ordervals = sale_order_obj._convert_to_write({name: new_record[name] for name in new_record._cache})
        fpos = ordervals.get('fiscal_position_id',fpos)

        """Changes by Dhruvi payment_term_id and auto_workflow_id is fetched according to seller wise"""
        ordervals.update({
            'company_id':instance.company_id.id,
            ...
            'auto_workflow_process_id':instance.seller_id.auto_workflow_id.id,
        ...
        """Condition changed by Dhruvi
            is_default_odoo_sequence_in_sale_order, order_prefix is being fetched according to seller wise"""
        if not instance.seller_id.is_default_odoo_sequence_in_sales_order:
            ordervals.update({'name':"%s%s" %(instance.seller_id.order_prefix and instance.seller_id.order_prefix+'_' or '',amazon_order_ref)})
...
#         else:
        ...
        if not 'invoice_address' in return_partner:
    ...
    def process_sale_order_xml_report_file(self,orders_data):
        ...
        for x in range(0, len(orders_data), 50):
            ...
            for order in orders:
                ...
                already_exist_amazon_order=sale_order_obj.search([('amz_instance_id','in',instance_ids),('amazon_reference','=',amazon_order_ref)])
                if already_exist_amazon_order:
                    continue
    ...
    def create_sale_order_by_xml_report(self,order_data,instance):
        ...
        amazon_order = self.env['sale.order'].create(order_vals)
        
        for order_line in order_lines:
            sale_line_obj.create_sale_order_line_by_xml_report(order_line,instance,amazon_order)
    ...
    def create_sales_order_vals_by_xml_report(self,partner_dict,order,instance):
        ...
        """Condition changed by Dhruvi is_default_odoo_sequence_in_sale_order is fetched according to seller wise."""
        if not instance.seller_id.is_default_odoo_sequence_in_sales_order:
...
#             else:
        ...
        partnervals = {
                ...
                'country_id' : country and country.id,'phone' :phone,'zip' : bill_postal_code,'state_id' : state_id,'email':email,'parent_id':partner_id,
                'lang':instance.lang_id and instance.lang_id.code,'company_id':instance.company_id.id
            }
        if instance.customer_is_company and not partner_id:
            partnervals.update({'is_company':True})
...
#                     state_id=result_state.id
            ...
            partnervals = {
                    ...
                    'country_id' : country and country.id,'phone' :ship_phone,'zip' : ship_postal_code,'state_id' : state_id,'email':email,'parent_id': partner_id or invoice_partner.id,
                    'lang':instance.lang_id and instance.lang_id.code,'company_id':instance.company_id.id
                }
            if instance.pricelist_id:
                partnervals.update({'property_product_pricelist':instance.pricelist_id.id})
