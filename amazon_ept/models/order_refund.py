            ...
            for picking in self.order_id.picking_ids:
                ...
                for line in self.amazon_refund_line_ids:
                    if line.amazon_order_line_id:
                        move=env_thread1['stock.move'].search([('sale_line_id','=',line.amazon_order_line_id.id),('product_id','=',line.product_id.id),('picking_id','=',picking.id)])
...
#                                 move_dest_exists.append([0,0,exist_line])
                    ...
                    record=env_thread1['stock.return.picking'].create({'picking_id':picking.id,'move_dest_exists':move_dest_exists,'product_return_moves':product_return_moves,'location_id':result.get('location_id')})
    ...
    def _get_instance(self):
        if self.order_id:
            self.instance_id = self.order_id.amz_instance_id and self.order_id.amz_instance_id.id or False   
    ...
    def create_flat_file(self):
        ...
        file_order_ship.write(("order-id\torder-item-id\tadjustment-reason-code\tcurrency\titem-price-adj\t").encode())
        file_order_ship.write(("item-tax-adj\tshipping-price-adj\tshipping-tax-adj\tgift-wrap-price-adj\t").encode())
        file_order_ship.write(("gift-wrap-tax-adj\titem-promotion-adj\titem-promotion-id\tship-promotion-adj\t").encode())
        ...
        for line in self.amazon_refund_line_ids:
            ...
            file_order_ship.write(("%s\t%s\t%s\t%s\t%s\t"%(order_ref,line.amazon_order_line_id.amazon_order_item_id,line.message,currency,order_amount)).encode())
            file_order_ship.write(("%s\t%s\t%s\t%s\t"%(order_line_tax,shipping_charge,shipping_tax,gift_wrap_charge)).encode())
            file_order_ship.write(("%s\t%s\t%s\t%s\t"%(gift_wrap_tax,item_promotion_adjust,line.item_promotion_id or '',shipping_promotion_adjust)).encode())
            file_order_ship.write(("%s\t%s"%(line.shipping_promotion_id or '',qty_canceled)).encode())
    ...
    def create(self,vals):
        ...
        if vals.get('amazon_refund_line_ids',False):
            ...
            amazon_refund_line_ids=amazon_refund_line_ids and amazon_refund_line_ids[0] and amazon_refund_line_ids[0][1]    
    ...
    def write(self,vals):
        amazon_refund_lines_obj=self.env['amazon.refund.order.lines']
        amazon_refund_line_ids=vals.get('amazon_refund_line_ids',False)
        order_id=vals.get('order_id',False)
        if amazon_refund_line_ids:            
            vals.pop('amazon_refund_line_ids',False) 
            amazon_refund_line_ids=amazon_refund_line_ids and amazon_refund_line_ids[0] and amazon_refund_line_ids[0][1]
            record=amazon_refund_lines_obj.browse(amazon_refund_line_ids)
            lines=amazon_refund_lines_obj.search([('amazon_refund_id','=',self.id)])
            for line in lines:
                line.amazon_refund_id=False

            record.update({'amazon_refund_id':self.id})
            self.order_id=self.env['sale.order'].browse(order_id)
        super(amazon_order_refund_ept,self).write(vals)
