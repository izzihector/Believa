...
class order_return_config(models.Model):
    ...
    location_id = fields.Many2one('stock.location',string='Location',required=False,domain=[('usage','in',['internal','inventory'])])
    condition = fields.Selection([
        ('SELLABLE','SELLABLE'),('DAMAGED','DAMAGED'),
        ('CUSTOMER_DAMAGED','CUSTOMER DAMAGED'),('DEFECTIVE','DEFECTIVE'),
        ('CARRIER_DAMAGED','CARRIER DAMAGED'),('EXPIRED','EXPIRED'),('CUSTOMERDAMAGED','CUSTOMERDAMAGED')],
        string='Condition',required=True,copy=False)
    is_reimbursed = fields.Boolean("Is Reimbursed ?",default=False,help="Is Reimbursed?",copy=False)
