...
class instance_checking(sale_order):
    ...
    def import_sales_order(self,seller,marketplaceids=[],end_date=False,start_date=False):
        try:
            ept_key = self.env['ept.app.key.data'].search([('app','=','amazon_fba_connector')])
            if not ept_key:
        ...
        if len(ept_key) == 1:
            ...
            if check:
                return super(instance_checking,self).import_sales_order(seller=seller,marketplaceids=marketplaceids,created_before=end_date,created_after=start_date)
            else:
...
def decode_client_key(ept_key,db_name,app_name):
    ...
    if not ept_key.flag_date or (current_date-datetime.strptime(ept_key.flag_date,"%Y-%m-%d %H:%M:%S")).days >= 30:
