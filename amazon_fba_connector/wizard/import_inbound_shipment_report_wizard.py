...
class AmazonInboundShipmentReportWizard(models.TransientModel):
    ...
    choose_file = fields.Binary(string="Choose File",filters="*.csv",help="Select amazon In-bound shipment file.")
    file_name = fields.Char("Filename",help="File Name")
    delimiter=fields.Selection([('tab','Tab'),('semicolon','Semicolon'),('colon','Colon')],"Separator",default=False,help="Select separator type for the separate file data and import into ERP.")
    ...
    def import_inbound_shipment_report(self):
        ...
        if inbound_shipment_id and inbound_shipment_id.partnered_small_parcel_ids:
            return True
        ...
        if self.check_fields_validation(fields_name):
            ...
            for inboud_shipment_dict in inboud_shipment_data_list:
                ...
                if box_no in box_no_list:
                    if amazon_carton_content_info_dict and amazon_carton_content_info_dict.get(box_no) and seller_sku in amazon_carton_content_info_dict.get(box_no):
                        continue
                    amazon_product = amazon_product_ept.search([("seller_sku","=",seller_sku)],limit=1)
                    if amazon_product:
                        amazon_carton_content_info = False
                        amazon_carton_content_info = amazon_carton_content_info_obj.search([("amazon_product_id","=",amazon_product.id),("seller_sku","=",seller_sku),("quantity","=",quantity)],limit=1)
                        if not amazon_carton_content_info:
                            ...
                            amazon_carton_content_info = amazon_carton_content_info_obj.create(carton_details_vals)
                        if amazon_carton_content_info_dict and amazon_carton_content_info_dict.get(box_no) and seller_sku not in amazon_carton_content_info_dict.get(box_no):
                            amazon_carton_content_info and amazon_carton_content_info_dict.get(box_no).append(amazon_carton_content_info.id)
                        elif not amazon_carton_content_info_dict or not amazon_carton_content_info_dict.get(box_no):
                            amazon_carton_content_info and amazon_carton_content_info_dict.update({box_no:[amazon_carton_content_info.id]})
                else:
                    ...
                    vals = {
                        'ul_id': (product_ul.id if product_ul else (product_ul_id and product_ul_id.id)) or False,
                        'box_no': inboud_shipment_dict.get("box_no",""),
            ...
            if partnered_small_parcel_dict and box_no_list:
                for parcel_box_no in box_no_list:
                    if partnered_small_parcel_dict.get(parcel_box_no) and amazon_carton_content_info_dict.get(parcel_box_no):
                        partnered_small_parcel_dict.get(parcel_box_no).update({"carton_info_ids": [(6,0,amazon_carton_content_info_dict.get(parcel_box_no))]})
                        partnered_small_parcel_list.append((0,0,partnered_small_parcel_dict.get(parcel_box_no)))
                    else:
                        partnered_small_parcel_list.append((0,0,partnered_small_parcel_dict.get(parcel_box_no)))
