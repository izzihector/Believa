...
class import_product_inbound_shipment(models.TransientModel):
    ...
    """)
    delimiter=fields.Selection([('tab','Tab'),('semicolon','Semicolon'),('colon','Colon')],"Seperator",default="tab")    
    ...
    def wizard_view(self):
        view = self.env.ref('amazon_fba_connector.view_inbound_product_import_wizard')
...
#         open(new_file_name,'wb')
    ...
    def validate_process(self):
        ...
        for shipment in shipment_obj.browse(self.env.context.get('active_ids')):
            if shipment.state != "draft":
                raise except_orm(('Unable to process..!'), ('You can process with only draft shipment plan!.'))
            shipments.append(shipment)
            
        if len(shipment) > 1:
            raise except_orm(('Unable to process..!'), ('You can process only one shipment plan at a time!.'))
    ...
    def find_invalid_sku_list(self, product_data, shipment_plan):
        ...
        for data in product_data:
            ...
            product_domain = [('product_id','=',product.id),('instance_id','=',shipment_plan.instance_id.id),('fulfillment_by','=','AFN')]
    ...
    def import_shipment_line(self):
        ...
        if self.validate_fields(fieldname):
            ...
            if len(invalid_data) > 0:
                ...
                raise except_orm(('Invalid data found..!'), ('Please correct data first and then import file. \n%s.' %(msg)))
            ...
            for data in product_data:
                ...
                quantity = data.get('quantity') and float(data.get('quantity',0.0)) or 0.0
                quantity_in_case = data.get('quantity_in_case') and float(data.get('quantity_in_case',0.0))
                
                product = product_obj.search([('default_code','=',default_code)])
                product_domain = [('product_id','=',product.id),('instance_id','=',shipment_plan.instance_id.id),('fulfillment_by','=','AFN')]
                amazon_product_id = self.env['amazon.product.ept'].search(product_domain,limit=1)
                domain = [('amazon_product_id','=',amazon_product_id.id),('shipment_plan_id','=',shipment_plan.id)]
                ...
                try:
                    ...
                    raise except_orm(('Unable to process ..!'), ('Error found while importing products => %s.' %(str(e))))                    
