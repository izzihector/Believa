...
#     amazon_order_id = fields.Many2one('sale.order', string='Amazon Sales Reference',required=True,ondelete='cascade')
#     sale_order_line_id = fields.Many2one('sale.order.line', string='OE Order Line',  required=True,ondelete="cascade")
    ...
    line_tax_amount = fields.Float("Line Tax",default=0.0,digits=dp.get_precision("Product Price"))
#     instance_id=fields.Many2one("amazon.instance.ept",string="Instance",related="order_id.instance_id",required=True,readonly=True)
    ...
    amz_shipping_charge_ept = fields.Float("Shipping Charge",default=0.0,digits=dp.get_precision("Product Price"))
    amz_shipping_discount_ept = fields.Float("Shipping Discount",default=0.0,digits=dp.get_precision("Product Price"))
    amz_gift_wrapper_charge = fields.Float("Gift Wrapper Charge",default=0.0,digits=dp.get_precision("Product Price"))
    amz_promotion_discount = fields.Float("Promotion Discount",default=0.0,digits=dp.get_precision("Product Price"))    
    amz_shipping_charge_tax = fields.Float("Shipping Charge",default=0.0,digits=dp.get_precision("Product Price"))
    amz_order_line_tax = fields.Float("Order Line Tax",default=0.0,digits=dp.get_precision("Product Price"))
    amz_shipping_charge_tax = fields.Float("Shipping Charge Tax",default=0.0,digits=dp.get_precision("Product Price"))
    amz_gift_wrapper_tax = fields.Float("Tax of Shipping Charge",default=0.0,digits=dp.get_precision("Product Price"))
    ...
    def create_sale_order_line_by_xml_report(self,order_line,instance,amazon_order):
        ...
        if shipping_charge:
            ...
            """changes by Dhruvi 
            [fetching shipment_charge_product according to seller wise]"""
            
            if not shipment_product and instance.seller_id.shipment_charge_product_id:
            ...
            prod_order_line and prod_order_line.write({'amz_shipping_charge_ept':shipping_charge,'amz_shipping_charge_tax':shipping_tax})
        ...
        if total_promotion_discount<0.0:
            ...
            order_line_vals.update({'amz_promotion_claim_code':promotion_claim_code,'merchant_promotion_id':merchant_promotion_id})
            prod_order_line and prod_order_line.write({'amz_promotion_discount':total_promotion_discount,
                                                       'amz_promotion_claim_code':promotion_claim_code,
                                                       'amz_merchant_promotion_id':merchant_promotion_id
                                                       })
            self.create(order_line_vals)
...
#             'instance_id':instance.id,
        ...
        if amazon_product and amazon_product.product_asin:
            order_vals.update({'producturl':"%s%s"%(instance.producturl_prefix,amazon_product.product_asin)})
...
#         instance=self.order_id.amz_instance_id
#         if instance:
#             amazon_product_obj=self.env['amazon.product.ept']
#             amazon_product=amazon_product_obj.search([('instance_id','=',instance.id),('product_id','=',self.product_id.id)])
#             self.amazon_product_id=amazon_product and amazon_product[0] or False
#         
#         return res
#     @api.multi
#     def write(self,vals):
...
#             amazon_product=amazon_product_obj.search([('instance_id','=',instance.id),('product_id','=',product_id)])
#             vals.update({'amazon_product_id':amazon_product and amazon_product[0].id or False})
...
#         return res
    ...
    def create_sale_order_line_by_flat_report(self,order_line,instance,amazon_order):
        ...
        if gift_wrapper_charge:
            ...
            prod_order_line and prod_order_line.write({'amz_gift_wrapper_charge':gift_wrapper_charge,'amz_gift_wrapper_tax':tax_amount})
        ...
        if float(promotion_discount)<0.0:
            ...
            prod_order_line and prod_order_line.write({'amz_promotion_discount':promotion_discount,'amz_promotion_claim_code':promotion_claim_code})
            self.create(order_line_vals)
        ...
        if float(ship_promotion_discount)<0.0:
            ...
            """Changes by Dhruvi
                Getting value of promotion_discount_product)id according to seller wise"""       
            promotion_discount_product= instance.seller_id.promotion_discount_product_id
            ...
            prod_order_line and prod_order_line.write({'amz_promotion_discount':ship_promotion_discount,'amz_promotion_claim_code':ship_promotion_claim_code})
            self.create(order_line_vals)
...
#             'instance_id':instance.id,
            ...
            'amazon_order_item_id':order_line.get('order_item_id'),
            'discount':0.0,
            'amz_gift_wrap_type':order_line.get('gift_wrap_type'),
            'amz_gift_message_text':order_line.get('gift_message_text'),
            'amazon_product_id':amazon_product and amazon_product.id or False,
            'product_uom':new_record.product_uom.id
...
#         seller_sku and domain.append(('seller_sku','=',seller_sku))
    ...
    def create_sale_order_line(self,order_line,instance,amazon_order,create_service_line=True):
        ...
        if amazon_product:                          
            ...
            tax_id=False
            order_line_vals=self.create_sale_order_line_vals_amazon(order_line, qty_price_dict, tax_id, amazon_product,amazon_product.product_id and amazon_product.product_id.id, amazon_order, instance,title)
            order_line_vals.update({'line_tax_amount':tax_amount})
            prod_order_line=self.create(order_line_vals)
        ...
        if shipment_product or shipping_charge_description:                            
            ...
            order_line_vals=self.create_sale_order_line_vals_amazon(order_line, qty_price_dict, tax_id,False,shipment_product or False, amazon_order,instance,(shipping_charge_description and shipping_charge_description) or (shipment_product and shipment_product.name or 'Shipping Charge') )
            order_line_vals.update({'is_delivery':True})
            order_line_vals.update({'is_delivery':True,
                                    'amazon_product_id':amazon_product.id})
            prod_order_line and prod_order_line.write({'amz_shipping_charge_ept':shipping_charge,'amz_shipping_charge_tax':tax_amount})
            self.create(order_line_vals)
        ...
        if shipment_discount_product or shipping_discount_description:                            
            ...
            order_line_vals = self.create_sale_order_line_vals_amazon(order_line, qty_price_dict, False,False,shipment_discount_product or False, amazon_order,instance,(shipping_discount_description and shipping_discount_description) or (shipment_discount_product and shipment_discount_product.name or 'Shipping Discount'))
            prod_order_line and prod_order_line.write({'amz_shipping_discount_ept':shipping_discount})
            self.create(order_line_vals)
        ...
        if gift_wrapper_product or gift_wrapper_description:
            ...
            order_line_vals=self.create_sale_order_line_vals_amazon(order_line, qty_price_dict, tax_id,False,gift_wrapper_product  or False, amazon_order, instance,(gift_wrapper_description and gift_wrapper_description) or (gift_wrapper_product and gift_wrapper_product.name or 'Gift Wrapper Fee') )
            prod_order_line and prod_order_line.write({'amz_gift_wrapper_charge':git_wrapper_charge,'amz_gift_wrapper_tax':tax_amount})
            self.create(order_line_vals)
        ...
        if promotion_discount_product or promotion_discount_description:
            ...
            order_line_vals=self.create_sale_order_line_vals_amazon(order_line, qty_price_dict, tax_id,False,promotion_discount_product or False, amazon_order, instance,(promotion_discount_description and promotion_discount_description) or (promotion_discount_product and promotion_discount_product.name or 'Promotion Discount'))
            prod_order_line and prod_order_line.write({'amz_promotion_discount':promotion_discount})
            self.create(order_line_vals)                            
    ...
    def create_sale_order_line_vals_amazon(self,order_line,qty_price_dict,tax_id,amazon_product=False,odoo_product=False,amazon_order=False,instance=False,title=False):
