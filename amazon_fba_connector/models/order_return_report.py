...
class sale_order_return_report(models.Model):
    ...
    def create(self,vals):    
        try:
            sequence=self.env.ref('amazon_fba_connector.seq_import_customer_return_report_job')
            if sequence:
    ...
    def on_change_seller_id(self):
        value = {}
        if self.seller_id:
            value.update({'start_date':self.seller_id.return_report_last_sync_on,'end_date':datetime.now()})
        return {'value': value }
    
    @api.multi
    def request_report(self):
        self.ensure_one()
        self.env['report.request.history'].request_report(self.report_history_id,self.seller_id,self.report_type,self.start_date,self.end_date)
        return True
    @api.multi
    def get_report_request_list(self):
        self.ensure_one()
        self.env['report.request.history'].get_report_request_list(self.report_history_id)
        return True
    ...
    def auto_import_return_report(self,args={}):
        ...
        if seller_id:
            ...
            return_report = self.create({'report_type' : '_GET_FBA_FULFILLMENT_CUSTOMER_RETURNS_DATA_',
                         'seller_id':seller_id,
            ...
            self.env['report.request.history'].request_report(return_report.report_history_id,seller,return_report.report_type,return_report.start_date,return_report.end_date)
    ...
    def auto_process_return_order_report(self,args={}):
        seller_id = args.get('seller_id',False)
        if seller_id:
            report_obj = self.env['report.request.history']
            seller = self.env['amazon.seller.ept'].search([('id','=',seller_id)])
            ...
            for report in return_reports:
                report_obj.get_report_request_list(report.report_history_id)
            return_reports = self.search([('seller_id','=',seller.id),
                                          ('state','=','_DONE_'),
                                          ('auto_generated','=',True),
                                          ('report_id','!=',False)
    ...
    def get_report(self):
        ...
        if data:
            ...
            self.message_post(body=_("<b>Return Report Downloaded</b>"),attachment_ids=attachment.ids)
    ...
    def get_warehouse(self,fulfillment_center_id,seller,amazon_order):
        fulfillment_center = self.env['amazon.fulfillment.center'].search([('center_code','=',fulfillment_center_id),('seller_id','=',seller.id)],limit=1)
        warehouse = fulfillment_center and fulfillment_center.warehouse_id or amazon_order.amz_instance_id and amazon_order.amz_instance_id.fba_warehouse_id or amazon_order.amz_instance_id.warehouse_id or False
        return warehouse
    ...
    def get_set_product(self,move,product,flag=False):
        try :
            ...
            factor = from_uom._compute_quantity(move.sale_line_id.product_qty,to_uom) / bom_point.product_qty
            bom,lines = bom_point.explode(move.sale_line_id.product_id,factor,picking_type=bom_point.picking_type_id)        
            return lines
        except:
            return {}
                
    @api.multi
    def process_kit_type_product(self,move_lines,product,amazon_order,job,returned_qty,warehouse,return_date,sku,disposition,reason,return_picking_dict,status,fulfillment_center_id,remaning_move_qty):
        amazon_transaction_obj=self.env['amazon.transaction.log']
        skip_moves=[]
        for move in move_lines:
            ...
            if picking.state != 'done':
                ...
                log_line_vals.update( {
                                 ...
                                 'message' :'Order %s Is Skipped due to delivery picking  found in %s state '%(picking.state),
                                 'job_id':job.id
                                 })
            ...
            if bom_qty==0.0:
                continue            
            key = (move,warehouse,return_date,sku,disposition,reason,status,fulfillment_center_id)
    ...
    def process_return_report_file(self):
        ...
        for line in reader:
            ...
            amazon_products=amazon_product_obj.search([('seller_sku','=',sku),('instance_id','in',instance_ids)])
            if not amazon_products:
                log_line_vals=self.get_log_vals()
                log_line_vals.update( {
                                 'amazon_order_reference':amazon_order_id,
                                 'message' :'Order %s Is Skipped due to Product %s not found in ERP'%(amazon_order_id,sku),
                                 'job_id':job.id
                                 })
                amazon_transaction_obj.create(log_line_vals)
                continue                
            domain = [('order_id','in',amazon_orders.ids),('amazon_product_id','in',amazon_products.ids)]
            ...
            if not warehouse:
                ...
                log_line_vals.update( {
                                 'amazon_order_reference':amazon_order_id,
                                 'message' :'Order %s Is Skipped due warehouse not found in ERP || Fulfillment center %s '%(amazon_order_id,fulfillment_center_id),
                                 'job_id':job.id
                                 })
                amazon_transaction_obj.create(log_line_vals)          
                continue      
            ...
            for move_line in move_lines:
                ...
                if fba_returned_date:                
                    fba_returned_date=datetime.strftime(parser.parse(fba_returned_date),'%Y-%m-%d')
            ...
            if not move_lines:
                domain = [('picking_id', 'in',outgoing_picking_ids),('sale_line_id.product_id', '=',product.id),('state','=','done'),
                          ('id','not in',exclude_move_ids)]
                move_lines = move_obj.search(domain,order='product_qty desc')
                if move_lines:
                    return_picking_dict,remaning_move_qty=self.process_kit_type_product(move_lines,product,amazon_orders[0],job,returned_qty,warehouse,return_date,sku,disposition,reason,return_picking_dict,status,fulfillment_center_id,remaning_move_qty)
                    continue            
            if not move_lines:
                log_line_vals=self.get_log_vals()
                log_line_vals.update( {
                                 'amazon_order_reference':amazon_order_id,
                                 'message' :'Order %s Is Skipped due to delivery move not found either move have already returned or move missing in the ERP '%(amazon_order_id),
                                 'job_id':job.id
                                 })
                amazon_transaction_obj.create(log_line_vals)                
                continue
            ...
            if picking.state != 'done':
                ...
                amazon_transaction_obj.create(log_line_vals)                
                continue
            qty = move.product_qty - sum(move.move_dest_ids.filtered(lambda m: m.state in ['partially_available', 'assigned', 'done']).\
    ...
    def _create_fba_returns(self,return_picking_dict):
        ...
        for picking,key_qty in return_picking_dict.items():
            ...
            default_pick_type_id = picking.picking_type_id.return_picking_type_id and picking.picking_type_id.return_picking_type_id.id or picking.picking_type_id.id
            location_wise_moves = {}
            for key,qty in key_qty.items():
                move,warehouse,return_date,sku,disposition,reason,status,fulfillment_center_id= key
                move_dest_ids=[move_dest_id for move_dest_id in move.origin_returned_move_id.move_dest_ids if move_dest_id.state!='cancel']
                ...
                fulfillment_center=self.env['amazon.fulfillment.center'].search([('center_code','=',fulfillment_center_id),('seller_id','=',self.seller_id.id)])
                new_move = move.copy(default={
                                ...
                                'warehouse_id': warehouse and warehouse.id or picking.picking_type_id.warehouse_id.id,
