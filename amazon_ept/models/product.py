...
#         res={}
#         domain={'amazon_browse_node_id':[('country_id','=',self.instance_id.country_id.id),('is_root_category','=',False)],'amazon_browse_node_id2':[('country_id','=',self.instance_id.country_id.id),('is_root_category','=',False)]}
#         res['domain']=domain
#         return res
...
#     delivery_channel=fields.Selection([('in_store','In Store'),('direct_ship','Direct Ship')],default='direct_ship')
    ...
    """fields used_for_ids,target_audience_ids,subject_content_ids,other_attribute_ids,platinum_keyword_ids
...
#     serial_number_required=fields.Boolean("Serial Number Required",help="Indicates whether the product must have a serial number")
    ...
    brand=fields.Char(string="Product Brand",related='product_id.product_tmpl_id.product_brand_id.name',readonly=True)
...
#     tsd_language_ids=fields.Many2many('amazon.tsd.language.ept','amazon_tsd_language_rel','product_id','language_id',string="TSDLanguage")
    ...
    def export_product_amazon(self,instance,amazon_products):
        ...
        try:
            results=mws_obj.submit_feed(data.encode('utf-8'),'_POST_PRODUCT_DATA_',marketplaceids=[instance.market_place_id],instance_id=instance.id)
        ...
        if data:
            try:
                results=mws_obj.submit_feed(data.decode(),'_POST_PRODUCT_RELATIONSHIP_DATA_',marketplaceids=[instance.market_place_id],instance_id=instance.id)
            except Exception as e:
                raise Warning(str(e)) 
        return True   
    @api.multi
    ...
    def update_images(self,instance):
        ...
        if message_information:
            ...
            mws_obj=Feeds(access_key=str(instance.access_key),secret_key=str(instance.secret_key),account_id=str(instance.merchant_id),region=instance.country_id.amazon_marketplace_code or instance.country_id.code,proxies=proxy_data)        
            try:
                results=mws_obj.submit_feed(data.encode('utf-8'),'_POST_PRODUCT_IMAGE_DATA_',marketplaceids=[instance.market_place_id],instance_id=instance.id)
                results=results.parsed
                last_feed_submission_id=False
                if results.get('FeedSubmissionInfo',{}).get('FeedSubmissionId',{}).get('value',False):
                    last_feed_submission_id=results.get('FeedSubmissionInfo',{}).get('FeedSubmissionId',{}).get('value',False)
...
#        price = instance.pricelist_id.get_product_price(amazon_product.product_id,1.0,partner=False,uom_id=amazon_product.product_id.uom_id.id)
        price = pricelist_obj.get_product_price_ept(amazon_product.product_id,instance.pricelist_id.id,partner=False)
        price = price or 0.0
        seller_sku=amazon_product.seller_sku
    ...
    def update_price(self,instance):        
        ...
        if message_information:            
            ...
            mws_obj=Feeds(access_key=str(instance.access_key),secret_key=str(instance.secret_key),account_id=str(instance.merchant_id),region=instance.country_id.amazon_marketplace_code or instance.country_id.code,proxies=proxy_data)        
            try:
                results=mws_obj.submit_feed(data.encode('utf-8'),'_POST_PRODUCT_PRICING_DATA_',marketplaceids=[instance.market_place_id],instance_id=instance.id)
                results=results.parsed
                last_feed_submission_id=False
                if results.get('FeedSubmissionInfo',{}).get('FeedSubmissionId',{}).get('value',False):
                    last_feed_submission_id=results.get('FeedSubmissionInfo',{}).get('FeedSubmissionId',{}).get('value',False)
...
#         return subject_contents
    ...
    def get_description_data(self,product):
        ...
        if product.package_weight > 0.0:
            data.append("""<PackageWeight unitOfMeasure='%s'>%s</PackageWeight>"""%(product.package_weight_uom, str(round(float(product.package_weight), 2))))
          
        if product.shipping_weight>0.0:
            data.append("""<ShippingWeight unitOfMeasure='%s'>%s</ShippingWeight>"""%(product.shipping_weight_uom, str(round(float(product.shipping_weight), 2))))
...
#         return category_structure
    ...
    @api.multi
    def prepare_export_stock_level_dict(self,amazon_product,instance,message_information,message_id):
        stock = 0.00
        asin_qty = 1.00
        seller_sku=amazon_product.seller_sku
        ...
        if instance.stock_field:
            stock=prod_obj.get_stock_ept(amazon_product.product_id,instance.warehouse_id.id,amazon_product.fix_stock_type,amazon_product.fix_stock_value,instance.stock_field.name)
        else:
            stock=prod_obj.get_stock_ept(amazon_product.product_id,instance.warehouse_id.id,amazon_product.fix_stock_type,amazon_product.fix_stock_value)
    ...
    def export_stock_levels(self,instance,product_ids=[]):
        ...
        merchant_string ="<MerchantIdentifier>%s</MerchantIdentifier>"%(instance.merchant_id)
        for amazon_product in amazon_products:
            message_information=self.prepare_export_stock_level_dict(amazon_product,instance, message_information, message_id)
            message_id = message_id + 1            
            for child_product in amazon_product.child_variant_ids:
                message_id = message_id + 1                
                message_information=self.prepare_export_stock_level_dict(child_product, instance, message_information, message_id)                
        if message_information:
            ...
            mws_obj=Feeds(access_key=str(instance.access_key),secret_key=str(instance.secret_key),account_id=str(instance.merchant_id),region=instance.country_id.amazon_marketplace_code or instance.country_id.code,proxies=proxy_data)        
            try:
                feed_result=mws_obj.submit_feed(data.encode('utf-8'),'_POST_INVENTORY_AVAILABILITY_DATA_',marketplaceids=[instance.market_place_id],instance_id=instance.id)
