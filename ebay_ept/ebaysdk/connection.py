...
from odoo.addons.ebay_ept.ebaysdk import log
...
from odoo.addons.ebay_ept.ebaysdk import set_stream_logger, UserAgent
from odoo.addons.ebay_ept.ebaysdk.utils import getNodeText as getNodeTextUtils, smart_encode, smart_decode
from odoo.addons.ebay_ept.ebaysdk.utils import getValue, smart_encode_request_data
from odoo.addons.ebay_ept.ebaysdk.response import Response
from odoo.addons.ebay_ept.ebaysdk.exception import ConnectionError, ConnectionResponseError
...
class BaseConnection(object):
    ...
    # Custom eBay Log Method
    # Create By: Vora Mayur
    def api_call_log(self,data,action):
        if hasattr(oe_request,'env') and hasattr(oe_request,'cr'):
            cr, pool = oe_request.cr, oe_request.env
            
            if action == 'response':
                log_vals = {'message':'Ebay API Responce Log'}
                log_record_id = pool['ebay.log.book'].create(log_vals)
                file_name='ebay_response'+str(log_record_id)+'.txt'
            else:
                log_vals = {'message':'Ebay API Request Log'}
                log_record_id = pool['ebay.log.book'].create(log_vals)
                file_name='ebay_request'+str(log_record_id)+'.xml'
            
            attachment_vals = {
                                'name': file_name,
                                'datas': base64.b64encode(bytes(data,encoding="utf-8")),
                                'datas_fname': file_name,
                                'res_model': 'ebay.log.book',
                                'res_id':log_record_id
                            }
            attachment = pool['ir.attachment'].create(attachment_vals)
            cr.commit()
        return True
    ...
    def build_request(self, verb, data, verb_attrs, files=None):
        ...
        self.api_call_log(requestData, 'request')
        if files:
            del(headers['Content-Type'])
            
            #=======================
            # Change By Vora Mayur
            #=======================
            # Orignal Code (Before Change Orignal Code)
            # if isinstance(requestData, basestring):  # pylint: disable-msg=E0602
            #    requestData = {'XMLPayload': requestData}
            #
            # Change Code (After Change Orignal Code)
            # if isinstance(requestData, str):
            #    requestData = {'XMLPayload': requestData}
            
            if isinstance(requestData, str):
                requestData = {'XMLPayload': requestData}
            elif isinstance(requestData, basestring):  # pylint: disable-msg=E0602
                requestData = {'XMLPayload': requestData}
