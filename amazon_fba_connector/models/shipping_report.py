...
class shipping_report_request_history(models.Model):
    ...
    def on_change_seller_id(self):
        value = {}
        if self.seller_id:
            start_date=datetime.now() + timedelta(days=self.seller_id.fba_shipment_report_days * -1 or -3)
            value.update({'start_date':start_date,'end_date':datetime.now()})
        return {'value': value } 

    """Method added by Dhruvi[10-09-2018]
        It will not confirm sale order as they are already confirmed while processing shipment report.
    ...
    def auto_import_shipment_report(self,args={}):
        ...
        if seller_id:
            ...
            if seller.is_another_soft_create_fba_shipment:
                wizard_record=self.env['amazon.list.shipping.report.wizard'].create(
                                                                {'seller_id':seller_id,
                                                                 'start_date':start_date,
                                                                 'end_date':date_end}
                                                                )
                wizard_record.get_shipping_report()
            else:
                ship_report = self.create({'report_type' : '_GET_AMAZON_FULFILLED_SHIPMENTS_DATA_',
                             'seller_id':seller_id,
                             'start_date' : start_date,
                             'end_date' : date_end,
                             'state' :'draft',
            ...
            seller.write({'shipping_report_last_sync_on':date_end})            
        return True    
    @api.model
    def auto_process_shipment_report(self,args={}):
        ...
        if seller_id:
            ...
            shipment_reports = self.search([('seller_id','=',seller.id),('state','in',['_SUBMITTED_','_IN_PROGRESS_'])])
            if shipment_reports:
                total_length=len(shipment_reports.ids)
                for x in range(0,total_length,20):
                    reports = shipment_reports[x:x +20]
                    reports.get_report_request_list_via_cron(seller)
    ...
    def check_product_exist_or_not_create_based_on_configuration(self,row,job_ids):
        ...
        if not amazon_product:
            ...
            if erp_product:
                ...
                log_message = 'Odoo Product is already exists. System have created new Amazon Product %s for %s instance'%(sku, instance.name )
                ...
                log_action_type = 'create'
                log_message = 'System have created new Odoo Product %s for %s instance'%(sku, instance.name )
            ...
            if not skip_order:
                ...
                if not erp_product:
                    prod_vals.update({'name':product_name,'default_code':sku,'company_id':instance.company_id.id})
            
                amazon_product = amazon_product_obj.create(prod_vals)
                
            job=process_log_book_obj.search([('id','in',job_ids),('instance_id','=',instance.id)])
            if not job:
                job_log_vals = {
            ...
            log_line_vals = {
                             'model_id' : self.env['amazon.transaction.log'].get_model_id('shipping.report.request.history'),
                             'res_id' : self.id or 0,
                             'log_type' : 'not_found',
    ...
    @api.multi
    def create_or_update_outbound_order(self,order_details_dict,order_lines_details_dict,odoo_available_order_line_dict,odoo_available_order_line_service_dict):
        ...
        for order_wh,amazon_order in order_details_dict.items():
            ...
            if amazon_order.id in already_taken_orders:
                new_amazon_order = self.copy_amazon_order(amazon_order,warehouse,fulfillment_instance)
                new_amazon_order.write({'amz_fulfillment_instance_id':fulfillment_instance.id,'date_order':amazon_order.date_order})
                ...
                amazon_order.write({'auto_workflow_process_id':fulfillment_instance.seller_id.fba_auto_workflow_id.id,'amz_shipment_report_id':self.id})
        ...
        for wh_order_item_key,line_data in order_lines_details_dict.items():
            ...
            amazon_product = amazon_product_obj.search_amazon_product(fulfillment_instance.id,seller_sku,'AFN')
            product=amazon_product and amazon_product.product_id
            price_unit=odoo_available_order_line_dict.get(amazon_order,{}).get(product,{}).get('price',0.0)
            line_qty=odoo_available_order_line_dict.get(amazon_order,{}).get(product,{}).get('qty',0.0)
            odoo_available_order_line_dict.get(amazon_order,{}).get(product,{}).update({'qty':line_qty-line_data.get('shipped_qty',0.0)})
            discount=odoo_available_order_line_dict.get(amazon_order,{}).get(product,{}).get('discount',0.0)
            ...
            amazon_line_rec=amazon_product and order_line_obj.search([('order_id','=',amazon_splitedorder.id),('product_id','=',amazon_product.product_id.id),('amz_fulfillment_center_id','=',fulfillment_center.id)])
            if not amazon_line_rec:
                amazon_line_rec = self.create_sale_order_line(amazon_splitedorder,order_line_data,is_outbound_order=True)
            else:
                amazon_line_rec.write({'product_uom_qty':amazon_line_rec.product_uom_qty+line_data.get('shipped_qty',0.0)})
            
            #This method is for add shipping charge in  split order
            for product,line in odoo_available_order_line_service_dict.get(amazon_order,{}).items():
            ...
            for product,line in line_data.items():
                if line.get('qty')>0.0:
                    if not new_pending_quotations.get(amazon_order):
                        new_pending_quotation=self.copy_amazon_order(amazon_order,line.get('warehouse_id'),line.get('instance_id'))
...
#                         'amazon_product_id':amazon_product.id,
    ...
    def process_outbound_orders(self,outbound_order_rows):
        ...
        for row in outbound_order_rows:
            ...
            amazon_order=order_obj.search([('amazon_reference','=',odoo_sale_order_id),('state','=','draft')])            
            if not amazon_order:
                log_message='Order %s not found in ERP.'%(odoo_sale_order_id)
                if not amazon_transaction_obj.search([('message','=',log_message),('manually_processed','=',False)]):
                    if not job:                    
                        job_log_vals = {
                                        'skip_process' :True,
                                        'application' : 'sales',
            ...
            if amazon_order not in odoo_available_order_line_dict:
                ...
                for line in amazon_order.order_line:
                    qty=odoo_available_order_line_dict.get(amazon_order,{}).get(line.product_id.id,{}).get('qty',0.0)
                    if line.product_id.type!='service':
                        odoo_available_order_line_dict.get(amazon_order).update({line.product_id:{'qty':line.product_uom_qty+qty,'price':line.price_unit,'discount':line.discount,'warehouse_id':amazon_order.warehouse_id,'instance_id':amazon_order.instance_id}})
                    else:
                        odoo_available_order_line_service_dict.get(amazon_order).update({line.product_id:{'qty':line.product_uom_qty+qty,'price':line.price_unit,'discount':line.discount}})                    
            fulfillment_instance=amazon_order.amz_instance_id
            ...
            line_data = {
                         ...
                         'product_name':row.get('product-name',''),
                         'gift_wrapper_charge' : row.get('gift-wrap-price',0.0) and float(row.get('gift-wrap-price',0.0)) or 0.0,
            ...
            if wh_order_item_key in order_lines_details_dict:
                order_lines_details_dict[wh_order_item_key]['shipped_qty']+=line_data.get('shipped_qty',0.0)
                order_lines_details_dict[wh_order_item_key]['gift_wrapper_charge']+=line_data.get('gift_wrapper_charge',0.0)
                order_lines_details_dict[wh_order_item_key]['gift_wrapper_tax']+=line_data.get('gift_wrapper_tax',0.0)
                order_lines_details_dict[wh_order_item_key]['promotion_discount']+=line_data.get('promotion_discount',0.0)
            else:
        ...
        odoo_order_ids,odoo_order_wh_dict=self.create_or_update_outbound_order(order_details_dict,order_lines_details_dict,odoo_available_order_line_dict,odoo_available_order_line_service_dict)             
        ...
        for order_id in order_ids:
            try:
                ...
                continue
#         odoo_order_ids and self.env['sale.workflow.process.ept'].auto_workflow_process(ids=odoo_order_ids)
        odoo_order_ids and self.fba_auto_workflow_process(ids=odoo_order_ids)
        odoo_order_ids and self.process_delivery_orders_new(shipment_details_dict,odoo_order_wh_dict)            
        return True
    ...
    def re_process_shipment_file(self):
        ...
        records=amazon_transaction_obj.search([('model_id','=',model_id),('action_type','!=','create'),('res_id','=',self.id)])
        records.unlink()
#         self.with_context({'re_process_file':True}).process_shipment_file()
    ...
    def process_shipment_file(self):
        ...
        for row in reader:    
            order_exist = False
            skip_order,job_ids=self.check_product_exist_or_not_create_based_on_configuration(row,job_ids)
            if skip_order:
                continue
            ...
            if not instance:
                ...
                log_line_vals = {
                                 ...
                                 'message' :'Order skiped due to instance not found in ERP for sales channel %s'%(sales_channel),
                                 'job_id':job.id
                                 }
                amazon_transaction_obj.create(log_line_vals)
                continue    
            ...
            if fulfillment_id not in fulfillment_warehouse:
                ...
                if not fn_warehouse:
                    ...
                    log_line_vals = {
                                     ...
                                     'skip_record' : True,
                                     'amazon_order_reference':row.get('amazon-order-id',False),
                                     'message' :'Order skiped due to warehouse not found in ERP for fulfillment center %s'%(fulfillment_id),
                                     'job_id':job.id
                                     }
                    amazon_transaction_obj.create(log_line_vals)
                    continue                    
            ...
            order_wh_key = (amazon_order_id,warehouse)
            amazon_orders = order_obj.search([('amazon_reference','=',amazon_order_id),('amz_instance_id','=',instance.id)], order="id desc")
            flag=True
            for amazon_order in amazon_orders:
                for picking in amazon_order.picking_ids:
                    if picking.amazon_shipment_id==shipment_id:
                        flag=False
                        break
            if not flag:
                continue                        
            if order_wh_key not in order_details_dict:
                ...
                if amazon_order:
                    row.update({'instance':instance,'amazon_order':amazon_order,'fulfillment_center':fullfillment_center,'warehouse':warehouse})
            ...
            line_data = {
                         ...
                         'shipping_charge' : row.get('shipping-price',0.0) and float(row.get('shipping-price',0.0)) or 0,
                         'shipping_tax' : row.get('shipping-tax',0.0) and float(row.get('shipping-tax',0.0)) or 0,
                         'shipping_discount' :row.get('ship-promotion-discount',0.0) and float(row.get('ship-promotion-discount',0.0)) or 0,
                         'gift_wrapper_charge' : row.get('gift-wrap-price',0.0) and float(row.get('gift-wrap-price',0.0)) or 0,
                         'gift_wrapper_tax' : row.get('gift-wrap-tax',0.0) and float(row.get('gift-wrap-tax',0.0)) or 0,
                         'promotion_discount' : row.get('item-promotion-discount',0.0) and float(row.get('item-promotion-discount',0.0)) or 0,
            ...
            if wh_order_item_key in order_lines_details_dict:
                order_lines_details_dict[wh_order_item_key]['item_price']+=line_data.get('item_price',0.0)
                order_lines_details_dict[wh_order_item_key]['shipped_qty']+=line_data.get('shipped_qty',0.0)
                order_lines_details_dict[wh_order_item_key]['tax_amount']+=line_data.get('tax_amount',0.0)
                order_lines_details_dict[wh_order_item_key]['shipping_charge']+=line_data.get('shipping_charge',0.0)
                order_lines_details_dict[wh_order_item_key]['shipping_tax']+=line_data.get('shipping_tax',0.0)
                order_lines_details_dict[wh_order_item_key]['shipping_discount']+=line_data.get('shipping_discount',0.0)
                order_lines_details_dict[wh_order_item_key]['gift_wrapper_charge']+=line_data.get('gift_wrapper_charge',0.0)
                order_lines_details_dict[wh_order_item_key]['gift_wrapper_tax']+=line_data.get('gift_wrapper_tax',0.0)
                order_lines_details_dict[wh_order_item_key]['promotion_discount']+=line_data.get('promotion_discount',0.0)
            else:
        ...
        for order_wh,row in new_order_details_dict.items():
            ...
            if order_ref in new_order_references:
                amazon_order = order_obj.search([('amazon_reference','=',order_ref),('amz_instance_id','=',instance.id)], order="id desc")
                if amazon_order and amazon_order[0].warehouse_id.id==warehouse.id:
                    ...
                    new_amazon_order.write({'amz_instance_id':instance.id,'date_order':amazon_order[0].date_order})
        ...
        for order_wh,row in exist_order_details_dict.items():
            ...
            if len(amazon_order) == 1 and amazon_order.state in ['draft','sent']:
                if amazon_order.id in already_taken_orders:
                    ...
                    new_amazon_order.write({'amz_instance_id':instance.id,'date_order':amazon_order.date_order})
                    already_taken_orders.append(new_amazon_order.id)
                    odoo_order_wh_dict.update({order_wh:new_amazon_order})
                    odoo_order_ids.append(new_amazon_order.id)                    
                else:
        ...
        for wh_order_item_key,line_data in exist_order_lines_details_dict.items():
            ...
            if not amazon_order:
                continue
            shipped_qty = line_data.get('shipped_qty',0.0)
            amazon_line_rec = order_line_obj.search([('amazon_order_item_id','=',amazon_order_item_id),('order_id','=',amazon_order.id)])
            if not amazon_line_rec:
                amazon_line_rec = order_line_obj.search([('amazon_order_item_id','=',amazon_order_item_id),('order_id.amazon_reference','=',amazon_order.amazon_reference)])
            if not amazon_line_rec:
            ...
            if round(shipped_qty,2) != round(amazon_line_rec.product_uom_qty,2):
                if not split_order_line_dict.get((amazon_order,warehouse),[]):
                    split_order_line_dict[(amazon_order,warehouse)] = {amazon_line_rec:shipped_qty} 
                else:
                    split_order_line_dict[(amazon_order,warehouse)].update({amazon_line_rec:shipped_qty})
        ...
        for order_wh, amazon_order in odoo_order_wh_dict.items():
            ...
            for order_item_id in order_item_ids:
                ship_item_ids += [order_item_id+'_ship',order_item_id+'_ship_discount',order_item_id+'_gift_wrap',order_item_id+'_promo_discount']
            order_item_ids += ship_item_ids     
            amazon_line_ship_recs = order_line_obj.search([('amazon_order_item_id','not in',order_item_ids),('order_id','=',amazon_order.id)])
    ...
    def get_order_sequence(self,amazon_order,order_sequence):
        order_obj=self.env['sale.order']
        new_name = "%s%s" %(amazon_order.amz_instance_id.seller_id.order_prefix and amazon_order.amz_instance_id.seller_id.order_prefix+'_' or '', amazon_order.amazon_reference)     
    ...
    def copy_amazon_order(self,amazon_order,warehouse,new_instance=False):
        workflow_id = amazon_order.auto_workflow_process_id and amazon_order.auto_workflow_process_id.id or False        
        new_default_value={'order_line' : None,'warehouse_id':warehouse.id,'auto_workflow_process_id' : workflow_id,'amz_shipment_report_id':self.id}
    ...
    def split_order(self,split_order_line_dict):
        ...
        for order, lines in split_order_line_dict.items():
            ...
            for line, shipped_qty in lines.items():
                line.copy(default = {'order_id' : new_amazon_order.id,'product_uom_qty':(line.product_uom_qty-shipped_qty)})
    ...
    def create_amazon_order(self,instance,warehouse,row):
        ...
        carrier_id = False
        amazon_exist_order=order_obj.search([('amazon_reference','=',row.get('amazon-order-id',''))],order="id desc",limit=1)
        order_name=False
        if not instance.seller_id.is_default_odoo_sequence_in_sales_order_fba:
            if amazon_exist_order:
                ...
                order_name="%s%s" %(instance.seller_id.order_prefix and instance.seller_id.order_prefix+'_' or '', row.get('amazon-order-id',''))
        if carrier_code:
            """Modified by Dhruvi shipment_charge_product_id is fetched according to seller wise"""
            carrier_id = self.get_shipping_method(carrier_code,instance.seller_id.shipment_charge_product_id)
        partner_dict = self.create_or_update_partner_amazon(instance,row)
        warehouse = warehouse or instance.fba_warehouse_id and instance.fba_warehouse_id or instance.warehouse_id
        date_order=False
        if row.get('purchase-date',False):
            date_order=parser.parse(row.get('purchase-date',False)).astimezone(utc).strftime('%Y-%m-%d %H:%M:%S')
        else:
        ...
        vals = {'company_id':instance.company_id.id,'partner_id':partner_dict.get('invoice_address'),
                    ...
                    'warehouse_id':warehouse.id, 'picking_policy':instance.seller_id.fba_auto_workflow_id and instance.seller_id.fba_auto_workflow_id.picking_policy or instance.picking_policy,
                    'date_order':date_order, 'pricelist_id':instance.pricelist_id.id or False, 
                    'payment_term_id':instance.seller_id.payment_term_id.id or False,'fiscal_position_id':False, 
                    'invoice_policy':instance.invoice_policy or False, 'team_id':instance.team_id and instance.team_id.id or False,
                    'client_order_ref':False,'carrier_id':carrier_id,'invoice_shipping_on_delivery':False
                }
            
        order_vals = sale_order_obj.create_sales_order_vals_ept(vals)
        order_vals.update(
            {
            'auto_workflow_process_id':instance.seller_id.fba_auto_workflow_id and instance.seller_id.fba_auto_workflow_id.id or instance.seller_id.auto_workflow_id and instance.seller_id.auto_workflow_id.id or False,
    ...
    def get_shipping_method(self, ship_method,ship_product):
        ...
        carrier = delivery_carrier_obj.search([('amazon_code', '=', ship_method)],limit=1)
        if not carrier:
    ...
    def create_or_update_partner_amazon(self,instance,row):
        ...
        vals = {
                'state_code':bill_state or False,'state_name': bill_state or False,
                'country_code' :bill_country or False ,'country_name' : bill_country or False,
                'name' : name, 'parent_id' : partner_id, 'street' : bill_add1,'street2' :False,'city' : bill_city,
                'phone' : phone,'email':email,'zip':bill_postal_code,'lang':instance.lang_id and instance.lang_id.code,
                'company_id':instance.company_id.id, 'type':False, 'is_company':False
            }
        ...
        if not ship_address_same:
            ...
            vals = {
                'state_code':ship_state or False,'state_name':ship_state or False,
                'country_code' :ship_country or False ,'country_name' : ship_country or False,
                'name' : ship_name, 'parent_id' : partner_id or invoice_partner.id, 'street' : ship_add1,'street2' :False,'city' : ship_city,
                'phone' : ship_phone,'email':email,'zip':ship_postal_code,'lang':instance.lang_id and instance.lang_id.code,
                'company_id':instance.company_id.id, 'type':False, 'is_company':False
            }
            ...
            for picking in pickings:
                for data in datas:
                    ...
                    if not picking.amazon_shipment_date or not picking.amazon_purchase_date or not picking.estimated_arrival_date:
                        picking_vals.update(
                                             {
                                               'estimated_arrival_date':data.get('estimated_arrival_date') and data.get('estimated_arrival_date') or False,
                                               'amazon_shipment_date':data.get('shipment_date') and data.get('shipment_date') or False,
                                               'amazon_purchase_date':data.get('purchase_date') and data.get('purchase_date') or False,
                                               'date_done':data.get('shipment_date') and data.get('shipment_date') or datetime.now(),
                                             }
                                            )
                    ...
                    if amazon_order.amz_instance_id.manage_multi_tracking_number_in_delivery_order:
                        package = quant_package_obj.search([('tracking_no','=',data.get('tracking_number',False))])
                        if not package:
                            package = quant_package_obj.create({'tracking_no':data.get('tracking_number',False)})

                    sku=data.get('sku')
                    amazon_product = amazon_product_obj.search_amazon_product(amazon_order.amz_instance_id.id,sku,'AFN')
                    product=amazon_product and amazon_product.product_id or False
                    if not product:                    
                        product=product_obj.search([('default_code','=',sku)],limit=1)
                    move_lines = move_obj.search([('picking_id','=',picking.id),('product_id','=',product.id),('state','in',('confirmed','assigned','partially_available'))])
                    is_kit_product=False
                    if not move_lines:
                        move_lines = move_obj.search([('picking_id','=',picking.id),('sale_line_id.product_id','=',product.id),('state','in',('confirmed','assigned','partially_available'))])                        
                        is_kit_product=True
                    if not move_lines:
                        continue
                            ...
                            if qty_left>0.0:
                                stock_move_line_obj.create(
                                    {       
                                            ...
                                            'product_uom_id':product_move_lines[0].product_id.uom_id.id, 
                                            'picking_id':picking.id,
                                            'qty_done':float(qty_left) or 0,
                                            'ordered_qty':float(op_qty) or 0,                                            
                                            'result_package_id':package and package.id or False,
...
#        pick_ids and  picking_obj.browse(list(set(pick_ids))).action_done()       
                ...
                for data in datas:
                    ...
                    if not picking.amazon_shipment_id or not picking.carrier_id:
                        ...
                        if amazon_order.carrier_id and amazon_order.carrier_id.amazon_code != data.get('carrier',''):
                            ...
                            carrier_id = self.get_shipping_method(data.get('carrier',''), amazon_order.amz_instance_id.seller_id.shipment_charge_product_id)
                            if carrier_id:
                                picking_vals.update({'carrier_id':carrier_id})
                    picking_vals and picking.write(picking_vals)
                    package=False
    ...
    def get_warehouse(self,fulfillment_center_id,instance):
        fulfillment_center = self.env['amazon.fulfillment.center'].search([('center_code','=',fulfillment_center_id),('seller_id','=',instance.seller_id.id)])
    ...
    def get_fiscal_position(self,warehouse,amazon_order,is_order=False):
        ...
        origin_country_id = warehouse_partner and warehouse_partner.country_id and warehouse_partner.country_id.id or False
        ctx = self._context.copy()
        ctx.update({'origin_country_ept':origin_country_id})
        fiscal_position = self.env['account.fiscal.position'].with_context(ctx).get_fiscal_position(amazon_order.company_id.id,partner_id,amazon_order.partner_shipping_id and amazon_order.partner_shipping_id.id) 
    ...
    def get_item_price(self,item_price,tax_amount):
        ...
        return item_price
    @api.multi 
    def calculate_tax_amount(self,item_price,tax_amount):
        return tax_amount
    ...
    def create_sale_order_line(self,amazon_order,line_data,is_outbound_order=False):
        ...
        tax_amount=self.calculate_tax_amount(item_price, tax_amount)
        ...
        if not is_outbound_order:
            qty_price_dict = order_line_obj.calculate_order_qty_and_price_based_on_asin_qty(amazon_product,float(item_price),shipped_qty)
            qty_price_dict.update({'amazon_order_qty':shipped_qty,'amazon_order_item_id':line_data.get('amazon_order_item_id')})
        else:
        ...
        order_line_vals = self.create_sale_order_line_vals(qty_price_dict, tax_ids, amazon_product,amazon_product.product_id and amazon_product.product_id.id, amazon_order, instance,title)
        ...
        fulfillment_center and order_line_vals.update({'amz_fulfillment_center_id':fulfillment_center.id})                    
    ...
    def update_sale_order_line(self,amazon_order,amazon_line_rec,line_data):
        ...
        qty_price_dict = order_line_obj.calculate_order_qty_and_price_based_on_asin_qty(amazon_product,item_price,shipped_qty)
    ...
    def create_order_charges_line(self,amazon_line_rec,line_data):
        ...
        shipping_tax=self.calculate_tax_amount(shipping_charge, shipping_tax)
        shipping_charge=self.get_item_price(shipping_charge, shipping_tax)
        if shipping_charge:
            shipment_product = False
            shipping_charge_description = ''
            amazon_line_ship_rec = order_line_obj.search([('amazon_order_item_id','=',amazon_line_rec.amazon_order_item_id+'_ship'),('order_id','=',amazon_order.id)])
            ...
            tax_id = False
            qty_price_dict.update({'order_qty':1,'amount_per_unit':shipping_charge,'amazon_order_qty':1,'amazon_order_item_id':amazon_line_rec.amazon_order_item_id+'_ship'})
            order_line_vals=self.create_sale_order_line_vals(qty_price_dict, tax_id,False,shipment_product and shipment_product.id or False, amazon_order,instance,(shipping_charge_description and shipping_charge_description) or (shipment_product and shipment_product.name) )
            order_line_vals.update({'is_delivery':True,
                                    'amazon_product_id':amazon_product.id,
                                    'line_tax_amount':shipping_tax})
            amazon_line_rec and amazon_line_rec.write({'amz_shipping_charge_ept':shipping_charge,'amz_shipping_charge_tax':shipping_tax})
        ...
        if shipping_discount:
            ...
            amazon_line_ship_rec = order_line_obj.search([('amazon_order_item_id','=',amazon_line_rec.amazon_order_item_id+'_ship_discount'),('order_id','=',amazon_order.id)])
            ...
            qty_price_dict.update({'order_qty':1,'amount_per_unit':-shipping_discount,'amazon_order_qty':1,'amazon_order_item_id':amazon_line_rec.amazon_order_item_id+'_ship_discount'})
            order_line_vals = self.create_sale_order_line_vals(qty_price_dict, False,False,shipment_product and shipment_product.id or False, amazon_order,instance,(shipping_discount_description and shipping_discount_description) or (shipment_product and shipment_product.name))
            order_line_vals.update({'amazon_product_id':amazon_product.id})
            
            amazon_line_rec and amazon_line_rec.write({'amz_shipping_discount_ept':-shipping_discount})
            if amazon_line_ship_rec:
                amazon_line_ship_rec.write(order_line_vals)
            else:    
                order_line_obj.create(order_line_vals)                
        ...
        gift_wrapper_tax=self.calculate_tax_amount(gift_wrapper_charge, gift_wrapper_tax)
        ...
        if gift_wrapper_charge:
            ...
            amazon_line_gift_wrap_rec = order_line_obj.search([('amazon_order_item_id','=',amazon_line_rec.amazon_order_item_id+'_gift_wrap'),('order_id','=',amazon_order.id)])
            ...
            tax_id=False
            qty_price_dict.update({'order_qty':1,'amount_per_unit':gift_wrapper_charge,'amazon_order_qty':1,'amazon_order_item_id':amazon_line_rec.amazon_order_item_id+'_gift_wrap'})
            order_line_vals=self.create_sale_order_line_vals(qty_price_dict, tax_id,False,gift_wrapper_product and gift_wrapper_product.id or False, amazon_order, instance,(gift_wrapper_description and gift_wrapper_description) or (gift_wrapper_product and gift_wrapper_product.name) )
            order_line_vals.update({
                                    'line_tax_amount':gift_wrapper_tax
                                    })
            amazon_line_rec and amazon_line_rec.write({'amz_gift_wrapper_charge':gift_wrapper_charge,'amz_gift_wrapper_tax':gift_wrapper_tax})
        ...
        if promotion_discount:
            ...
            amazon_line_promo_dis_rec = order_line_obj.search([('amazon_order_item_id','=',amazon_line_rec.amazon_order_item_id+'_promo_discount'),('order_id','=',amazon_order.id)])
            ...
            qty_price_dict.update({'order_qty':1,'amount_per_unit':promotion_discount, 'amazon_order_qty':1,'amazon_order_item_id':amazon_line_rec.amazon_order_item_id+'_promo_discount'})
            order_line_vals=self.create_sale_order_line_vals(qty_price_dict, False,False,promotion_discount_product and promotion_discount_product.id or False, amazon_order, instance,(promotion_discount_description and promotion_discount_description) or (promotion_discount_product and promotion_discount_product.name))
            amazon_line_rec and amazon_line_rec.write({'amz_promotion_discount':promotion_discount})
    ...
    @api.multi
    def create_sale_order_line_vals(self,qty_price_dict,tax_id,amazon_product=False,odoo_product_id=False,amazon_order=False,instance=False,title=False):
        ...
        odoo_product=odoo_product_id and self.env['product.product'].browse(odoo_product_id) or False
        ...
        if amazon_product and amazon_product.product_asin:
            order_vals.update({'producturl':"%s%s"%(instance.producturl_prefix or '',amazon_product.product_asin)})
